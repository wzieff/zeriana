...
function drawInfinitySymbol(cx, cy, ctx) {
    ...
    // ✅ Force `t` to start at 0 for a clean start
    const t = (elapsedTime * Math.PI * 2) % (Math.PI * 2);


    // Reset every 1.5 seconds to create a continuous flowing effect
    if (elapsedTime > 1.5) {
        infinityPoints.splice(0, maxTrailLength / 4); // ✅ Remove oldest points gradually instead of full reset
        startTime = Date.now(); 
    }
